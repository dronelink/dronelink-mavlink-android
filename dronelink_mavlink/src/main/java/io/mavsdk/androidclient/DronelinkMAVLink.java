...
public class DronelinkMAVLink {

    public static final double GimbalRotationMinTime = 0.1;
    public static final double DroneMaxVelocity = 15.0;

    public static CommandError createCommandError(final io.mavsdk.System error) {
        return error == null ? null : new CommandError(error.getTelemetry().getStatusText().toString(), 10101);
    }

    public static double getGPSSignalValue(final Flowable<Telemetry.GpsInfo> level) {
        if (level) {
            return 0.8;
        }
//        switch (level) {
