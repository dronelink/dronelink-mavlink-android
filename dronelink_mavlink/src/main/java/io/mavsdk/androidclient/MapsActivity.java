...
//    Original code - case statements are not allowed with R.id.X in mapbox version 9.7.2
...
//      drone.getAction().arm().andThen(drone.getAction().takeoff()).andThen(drone.getAction().land()).subscribe();
      drone.getAction().arm()
              .doOnComplete(() -> logger.debug("Arming...TESTIURA"))
              .andThen(drone.getAction().takeoff())
              .doOnComplete(() -> logger.debug("Takeoff...TESTIURA"))
              .delay(10, TimeUnit.SECONDS)
//              .andThen(drone.getOffboard().setPositionNed(new Offboard.PositionNedYaw(0f,0f,0f,0f)))
//              .andThen(drone.getOffboard().start())
//              .andThen(drone.getOffboard().setPositionNed(new Offboard.PositionNedYaw(2.0f,0f,0f,90f)))
              .andThen(drone.getOffboard().setVelocityBody(new Offboard.VelocityBodyYawspeed(0f,0f,0f,0f)))
              .andThen(drone.getOffboard().start())
              .andThen(drone.getOffboard().setVelocityBody(new Offboard.VelocityBodyYawspeed(5f,0f,0f,0f)))
              .delay(10, TimeUnit.SECONDS)
              .andThen(drone.getOffboard().setVelocityBody(new Offboard.VelocityBodyYawspeed(-5f,0f,0f,0f)))
              .delay(10, TimeUnit.SECONDS)
              .andThen(drone.getOffboard().setVelocityBody(new Offboard.VelocityBodyYawspeed(0f,0f,0f,45f)))
              .doOnComplete(() -> logger.debug("Offboard position...TESTIURA"))
              .delay(10, TimeUnit.SECONDS)
              .andThen(drone.getAction().land())
              .doOnComplete(() -> logger.debug("LAND...TESTIURA")).subscribe();
    } else {
      return super.onOptionsItemSelected(item);
    }
    return true;
